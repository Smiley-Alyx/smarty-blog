...
Простой полностью рабочий блог на чистом PHP 8.2, MySQL и Smarty без фреймворков.
...
ИИ использовался как инженерный ассистент для ускорения:

- генерации каркаса файлов и шаблонного кода;
- подготовки SQL-запросов и рефакторинга структуры;
- генерации SVG-плейсхолдеров для статей;
- проверки консистентности по этапам.

Архитектурные и технические решения принимались вручную, с валидацией запуска и поведения приложения в Docker.
